...
public class EditOrderController {
    ...
    private void saveOrder() {
        ...
        // Sincronizar tabla
        stopTableEditingIfNeeded();
        
        List<Article> updatedArticles = buildUpdatedArticlesFromTable();
        String orderId = orderToEdit.getId();

        Order orderInList = findOrderInListById(orderId);
        applyArticlesToOrder(orderInList, updatedArticles, orderId);
        ...
        try {
            saveOrdersAndFinalize(orderId, orderInList);
        } catch (IOException e) {
    ...
    private void stopTableEditingIfNeeded() {
    ...
    private List<Article> buildUpdatedArticlesFromTable() {
    ...
    private Order findOrderInListById(String orderId) {
    ...
    private void applyArticlesToOrder(Order orderInListOrNull, List<Article> updatedArticles, String orderId) {
        if (orderInListOrNull != null) {
            log.info("Before update - Order {} has {} articles", orderId, orderInListOrNull.getArticles().size());
            for (int i = 0; i < orderInListOrNull.getArticles().size() && i < updatedArticles.size(); i++) {
                Article old = orderInListOrNull.getArticles().get(i);
            ...
            orderInListOrNull.setArticles(updatedArticles);
            orderToEdit = orderInListOrNull;
            log.info("Order {} found in list and updated. Articles count: {}", orderId, updatedArticles.size());
            log.info("After update - Order {} has {} articles", orderId, orderInListOrNull.getArticles().size());
            for (Article art : orderInListOrNull.getArticles()) {
    ...
    private void saveOrdersAndFinalize(String orderId, Order orderInListOrNull) throws IOException {
        log.info("Saving {} orders to json file...", orders.size());
        log.info("Order to save - ID: {}, Articles: {}", orderId, orderInListOrNull != null ? orderInListOrNull.getArticles().size() : orderToEdit.getArticles().size());
        ...
        if (orderInListOrNull != null) {
            ...
            for (Article a : orderInListOrNull.getArticles()) {
